	...
	*/
	...
	if (is_dir_rx) {
		...
		if (add_to_list) {
			...
			ret = idrv->qos_ops->validate_tx_tc(idev, &ioss_qos_table.qos_tx_pending_table,
							    ioss_qos_new_nodes.tx_node);
			if (ret) {
				goto add_err;
			} else {
		...
		struct device_attribute *attr, const char *user_buf, size_t size)
{
	...
	if (strnstr(iface->ipa_config, "qos", IPA_ETH_CONFIG_LEN)) {
		...
		ioss_qos_dev_err(idev, "[ioss qos] : Received default/invalid IPA config. EMAC QOS Failed, routing all traffic to BE");
		return -EINVAL;
	}
	...
	if (res.qos_response_status != QOS_COMMIT_EMPTY)
