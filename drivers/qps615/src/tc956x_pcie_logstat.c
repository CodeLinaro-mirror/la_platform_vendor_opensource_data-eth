...
int tc956x_logstat_state_log_summary(void __iomem *pbase_addr, enum ports nport)
{
	...
	if (ret == 0) {
		...
		strscpy(cur_ltssm, ltssm_states[state], sizeof(cur_ltssm));
...
int tc956x_logstat_state_log_analyze(uint32_t cur_state)
{
	...
	strscpy(l1ss, state_L1[l1_substate], sizeof(l1ss));

	/* TxL0s Decoding */
	tx_l0s = curr_state_log_data.bitfield.fifo_read_value3;
	strscpy(txl0s_dec, tx_L0s_state[tx_l0s], sizeof(txl0s_dec));

	/* RxL0s Decoding */
	rx_l0s = curr_state_log_data.bitfield.fifo_read_value2;
	strscpy(rxl0s_dec, rx_L0s_state[rx_l0s], sizeof(rxl0s_dec));
	...
	if (ltssm <= LTSSM_MAX_VALUE)
		strscpy(ltssm_dec, ltssm_states[ltssm], sizeof(ltssm_dec));
	...
	if (timeout == LTSSM_TIMEOUT_OCCURRED)
		...
		snprintf(append_str, sizeof(append_str), "--> DL_Active:%d, Speed:Gen%d, Width:x%d, LTSSM:%s", DlActive, LinkSpeed, LinkWidth, ltssm_dec);
