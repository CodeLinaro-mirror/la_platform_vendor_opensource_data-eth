...
static int rtkphy_c45_read_autoNegoAbility(struct phy_device *phydev)
{
    uint32  phyData = 0;

    phyData = phy_read_mmd(phydev, MDIO_MMD_VEND2, 0xA412);
    if (phyData < 0)
        return phyData;
    linkmode_mod_bit(ETHTOOL_LINK_MODE_1000baseT_Full_BIT, phydev->advertising, phyData & BIT(9));
    linkmode_mod_bit(ETHTOOL_LINK_MODE_1000baseT_Half_BIT, phydev->advertising, phyData & BIT(8));

    phyData = phy_read_mmd(phydev, MDIO_MMD_AN, 16);
    if (phyData < 0)
        return phyData;
    linkmode_mod_bit(ETHTOOL_LINK_MODE_10baseT_Half_BIT,  phydev->advertising, phyData & BIT(5));
    linkmode_mod_bit(ETHTOOL_LINK_MODE_10baseT_Full_BIT,  phydev->advertising, phyData & BIT(6));
    linkmode_mod_bit(ETHTOOL_LINK_MODE_100baseT_Half_BIT, phydev->advertising, phyData & BIT(7));
    linkmode_mod_bit(ETHTOOL_LINK_MODE_100baseT_Full_BIT, phydev->advertising, phyData & BIT(8));
    linkmode_mod_bit(ETHTOOL_LINK_MODE_Pause_BIT,         phydev->advertising, phyData & BIT(10));
    linkmode_mod_bit(ETHTOOL_LINK_MODE_Asym_Pause_BIT,    phydev->advertising, phyData & BIT(11));

    phyData = phy_read_mmd(phydev, MDIO_MMD_AN, 32);
    if (phyData < 0)
        return phyData;
    linkmode_mod_bit(ETHTOOL_LINK_MODE_2500baseT_Full_BIT,  phydev->advertising, phyData & BIT(7));
    linkmode_mod_bit(ETHTOOL_LINK_MODE_5000baseT_Full_BIT,  phydev->advertising, phyData & BIT(8));
    linkmode_mod_bit(ETHTOOL_LINK_MODE_10000baseT_Full_BIT, phydev->advertising, phyData & BIT(12));
...
{
    int ret = 0, status = 0;
    uint32  phyData = 0, spd = 0;
    phydev->speed = SPEED_UNKNOWN;
    phydev->duplex = DUPLEX_UNKNOWN;
    phydev->pause = 0;
    phydev->asym_pause = 0;

    /* advertised value maybe changed by auto-down-speed */
    ret = rtkphy_c45_read_autoNegoAbility(phydev);
    if (ret)
        return ret;
    ...
    if (phyData < 0)
        return phyData;
    phydev->link = (phyData & BIT(2)) ? true : false;
    ...
    phydev->duplex = (phyData & 0x8)? DUPLEX_FULL : DUPLEX_HALF;
