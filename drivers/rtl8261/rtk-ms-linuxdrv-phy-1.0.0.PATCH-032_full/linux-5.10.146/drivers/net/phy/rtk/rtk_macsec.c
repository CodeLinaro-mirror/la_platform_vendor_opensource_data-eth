...
static int rtk_macsec_add_rxsa(struct macsec_context *ctx)
{
    ...
    memset(macsec_db->rxsa_stats[sa_id], 0, sizeof(rtk_macsec_rxsa_stats_t));
    return ret;
}
...
static int rtk_macsec_add_txsa(struct macsec_context *ctx)
{
    ...
    memset(macsec_db->txsa_stats[sa_id], 0, sizeof(rtk_macsec_txsa_stats_t));
...
int rtk_macsec_init(struct phy_device *phydev)
{
    ...
    if (priv->macsec == NULL)
        priv->macsec = kzalloc(sizeof(rtk_macsec_port_info_t), GFP_KERNEL);
    if (!priv->macsec)
        return -ENOMEM;
    memset(priv->macsec, 0, sizeof(rtk_macsec_port_info_t));
...
void rtk_macsec_exit(struct phy_device *phydev)
{
    uint32 i = 0;
    struct rtk_phy_priv *priv = phydev->priv;
    ...
    if (priv->macsec)
    {
        for (i = 0; i < RTK_MAX_MACSEC_SA_PER_PORT; i++)
        {
            if (priv->macsec->rxsa_stats[i])
            {
                kfree(priv->macsec->rxsa_stats[i]);
                priv->macsec->rxsa_stats[i] = NULL;
            }
            if (priv->macsec->txsa_stats[i])
            {
                kfree(priv->macsec->txsa_stats[i]);
                priv->macsec->txsa_stats[i] = NULL;
            }
        }
        kfree(priv->macsec);
        priv->macsec = NULL;
